...
def display_grid(grid, model):
    ...
    x_labels = list(set([float(coord[0]) for coord in coordinates]))
    y_labels = list(set([int(coord[1]) for coord in coordinates]))
    x_labels.sort()
    y_labels.sort()
    x_indices = [x_labels.index(float(coord[0])) for coord in coordinates]
    y_indices = [y_labels.index(int(coord[1])) for coord in coordinates]
